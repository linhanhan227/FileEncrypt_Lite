...
create_and_push_tag() {
    ...
    if [ -z "${current_commit:-}" ]; then
        error_return "无法获取当前提交记录，仓库可能没有提交"
        return 1
    fi
    ...
    if [ -z "$(git tag -l --sort=-v:refname | head -5)" ]; then
        echo "(暂无Tag)"
    fi

    # 输入tag名称
    read -p "请输入Tag名称（格式如 v1.0.0）：" tag_name
    if [ -z "$tag_name" ]; then
        error_return "Tag名称不能为空"
        return 1
    fi

    # 验证tag格式（基本验证）
    if ! [[ "$tag_name" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        warning "Tag格式不符合语义化版本规范（建议格式：v1.0.0）"
        read -p "是否继续使用此Tag名称？(y/n)：" confirm
        if ! [[ "$confirm" =~ ^[Yy]$ ]]; then
            echo "已取消创建Tag"
            read -p "按Enter键返回菜单..."
            return 1
    ...
    if git rev-parse "refs/tags/${tag_name}" >/dev/null 2>&1; then
        existing_commit=$(git rev-parse "refs/tags/${tag_name}" 2>/dev/null || echo "")
        if [ "$existing_commit" == "$current_commit" ]; then
            warning "Tag $tag_name 已存在且指向当前提交"
            read -p "Tag已存在且未变，是否强制推送（覆盖远程）？(y/n)：" confirm
            if [[ "$confirm" =~ ^[Yy]$ ]]; then
                echo "执行: git push origin $tag_name --force"
                if ! git push origin "$tag_name" --force; then
                    error_return "推送Tag失败"
                    return 1
                fi
                success "Tag $tag_name 强制推送成功"
                read -p "按Enter键返回菜单..."
                return 0
            else
                echo "已取消操作"
            ...
            warning "Tag $tag_name 已存在且指向其他提交: $existing_commit"
            read -p "是否强制覆盖此Tag？(y/n)：" confirm
            if ! [[ "$confirm" =~ ^[Yy]$ ]]; then
                echo "已取消创建Tag"
                read -p "按Enter键返回菜单..."
                return 1
    ...
    # 确认操作
    echo -e "\n即将执行以下操作："
    echo "  1. 在当前提交 $current_commit 上创建Tag: $tag_name"
    echo "  2. 推送Tag到远程仓库: git push origin $tag_name"
    ...
    # 创建Tag
    echo "执行: git tag $tag_name"
    if ! git tag "$tag_name"; then
        error_return "创建Tag失败"
        return 1
    fi
    success "Tag $tag_name 创建成功"

    # 推送Tag
    echo "执行: git push origin $tag_name"
    if ! git push origin "$tag_name"; then
        error_return "推送Tag失败"
        return 1
    fi
    success "Tag $tag_name 推送成功"
...
while true; do
    ...
    case $choice in
        ...
        12) create_and_push_tag ;;
        0)
            echo -e "\n${GREEN}感谢使用，再见！${NC}"
            exit 0
            ;;
        *)
            echo -e "${RED}无效选项，请输入0-12之间的数字${NC}"
            read -p "按Enter键继续..."
            ;;
    esac
done